...
import { bieniciHeaders, fetchWithRetry, randomDelay } from './http-client'

const BIENICI_API = 'https://www.bienici.com/realEstateAds.json'

const SEARCH_AREA = {
  zoneIdsByType: {
    zoneIds: ['38053', '38193', '38537', '38515', '38138', '38509', '38090', '38070']
  },
}
...
export async function scrapeBienIci(maxPages = 2): Promise<{ annonces: RawAnnonce[]; errors: string[] }> {
  ...
  for (let page = 1; page <= maxPages; page++) {
    try {
      ...
      const res = await fetchWithRetry(`${BIENICI_API}?${params}`, {
        headers: bieniciHeaders(),
        retries: 2,
        backoffMs: 2000,
      })
