...
import { ArrowLeft, ExternalLink, MapPin, Maximize, BedDouble, Zap, Clock, TrendingDown, Heart, Building2, ChevronRight } from 'lucide-react'
...
export default function AnnonceDetailClient() {
    ...
    return (
      <div className="p-4 md:p-8 max-w-4xl mx-auto">
        ...
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-8 text-center">
          <h1 className="text-xl text-brand-text mb-2">{error}</h1>
          <p className="text-sm text-brand-muted mb-4">Impossible de charger l&apos;annonce. Verifiez votre connexion et reessayez.</p>
          <button onClick={() => window.location.reload()} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-brand-gold text-brand-bg hover:bg-brand-gold-light">
            Reessayer
          </button>
        </div>
      </div>
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      {/* Breadcrumb */}
      <nav aria-label="Fil d'ariane" className="flex items-center gap-1 text-xs text-brand-muted mb-4">
        <Link href="/" className="hover:text-brand-gold">Accueil</Link>
        <ChevronRight size={12} />
        <Link href="/annonces" className="hover:text-brand-gold">Annonces</Link>
        <ChevronRight size={12} />
        {zone && (
          <>
            <span>{zone.name}</span>
        ...
        <span className="text-brand-text truncate max-w-[200px]">{annonce.title}</span>
      </nav>
              ...
              {annonce.photos.length > 1 && (
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {annonce.photos.map((photo, i) => (
                    ...
                    >
                      <Image src={photo} alt={`${annonce.title} - photo ${i + 1}`} fill sizes="80px" className="object-cover" loading="lazy" />
                    </button>
                  ))}
                </div>
              )}
